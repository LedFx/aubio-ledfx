/* 
 * Tempo Tracking Benchmark Test
 * 
 * This test measures:
 * 1. BPM detection accuracy against ground truth
...
void calculate_metrics(benchmark_results_t *results) {
    ...
    for (i = 0; i < results->num_sections; i++) {
        if (results->sections[i].detected) {
            ...
            /* Calculate response time (skip first section) */
            if (i > 0) {
...
int main(int argc, char **argv) {
    ...
    /* Allow override of test file */
    if (argc >= 2) {
        test_file = argv[1];
    }
    
    printf("Running tempo tracking benchmark on: %s\n", test_file);
    printf("Using window size: %d, hop size: %d\n", win_size, hop_size);
    
    /* Open audio file */
    aubio_source_t *source = new_aubio_source(test_file, samplerate, hop_size);
    if (!source) {
        fprintf(stderr, "Error: Could not open %s\n", test_file);
        fprintf(stderr, "Please run: python3 generate_tempo_test_audio.py\n");
        return 1;
    }
    ...
    int stable_count = 0;
    const int STABILITY_THRESHOLD = 5;  /* Frames needed for stable detection */
    ...
    do {
        ...
        /* Check for stable BPM detection */
        ...
        /* Record detection when BPM stabilizes */
